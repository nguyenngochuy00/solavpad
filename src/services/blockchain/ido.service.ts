...
import { IdoInfoType, UserStraitPda } from '../../types';
...
export class IdoUtil {
	fcfsTimestamp = (idoAccount: IdoInfoType): number => {
	...
	closeTimestamp = (idoAccount: IdoInfoType): number => {
		let ts = idoAccount.openTimestamp;
		const rounds = idoAccount.rounds;
		for (let i = 0; i < rounds.length; i++) {
			ts += rounds[i].durationSeconds;
	...
	isClosed = (currentTimestamp: number, idoAccount: IdoInfoType): boolean => {
		const { participated, cap, closed } = idoAccount;
		if (
			closed ||
			currentTimestamp >= this.closeTimestamp(idoAccount) ||
			participated >= cap
		)
			return true;
	...
	getAllocationRemaining = (
		...
		if (userPda.allocated) {
			...
			const allocated =
				idoAccount.rounds[roundIndex].tierAllocations[tierIndex];
			if (participated < allocated) return allocated.sub(participated);
		}

		return new BN(0);
	};
	idoInfo = (idoAccount: IdoInfoType, currentTimestamp: number) => {
